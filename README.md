...
Current framework version: `3.0.0`.
...
target: the verified bundle manifest, schemas, blank state templates, provenance, and
`ownership.json` plus its checksum. The ownership record contains only paths generated by
this framework and their checksums.
...
Use the validated transition command instead of editing status fields by hand:

```bash
python3 agents/framework.py transition \
  --state /path/to/project/.migration/state.json \
  --to discover \
...
Validate one artifact or the complete state graph:

```bash
python3 agents/framework.py validate-artifact .migration/state.json
python3 agents/framework.py validate-migration .migration
```
...
Completion uses profile-configured gates rather than a universal coverage percentage or
