...
1. Run `python3 agents/framework.py validate-migration .migration`.
...
5. Unless `--report-only` is set, transition `state.json` from `execute` to `verify` with
   `agents/framework.py transition`.
...
the exact project-root-relative working directory, the exact command as executed, toolchain
and runtime identifiers, exit status, and log/report artifact in an `EVID-NNNN` record with
`phase: verify` and this slice ID. Do not record a normalized or equivalent command from a
different directory; rerun the recorded command itself before assigning `status: pass`.
...
### Gate 6: Traceability completeness
...
This is a presence and cross-reference gate, not a semantic comparison. Do not score method
counts or declare meaning equivalent because names/signatures exist.
...
Create one immutable `.migration/evidence/EVID-NNNN.json` per independently reproducible gate.
Before promotion, rerun or mechanically check that its recorded working directory and command
name the command actually executed, its exit status supports the declared result, and every
recorded artifact exists at the recorded target-contained path. Then update traceability with
those IDs. A narrative report may be written to
